package rip.orbit.hub.scoreboard;
...
import rip.orbit.hub.util.TimeUtil;
            ...
            if (queue == null) {
                for (String line : uHub.getInstance().getConfig().getStringList("scoreboard.normal")) {
                    if (rank == null) {
                        ...
                        line = line.replaceAll("%rank%", uHub.getInstance().getConfig().getBoolean("scoreboard.rankcolored") ? rank.getFancyName() : rank.getName());
                    }
